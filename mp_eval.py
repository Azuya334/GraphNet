...
# load data
input_dims =5 #4 # WAS: 5 (removed layer_id)
...
def save_to_parquet(data, filepath, retries=5, delay=5):
    for attempt in range(retries):
        try:
            awkward.to_parquet(data, filepath)
            return
        except OSError as e:
            print(f"Attempt {attempt + 1} failed: {e}")
            time.sleep(delay)
    raise OSError(f"Failed to write parquet file after {retries} attempts.")

path = args.test_output_path
def run_one_file(filepath, extra_label,model,rank):
    ...
    test_preds = evaluate(model, test_loader, rank, return_scores=True)
    ...
    save_to_parquet(out_data, pred_file)


def run_filelist(bkg_list, rank):
    # model
    ...
    bkg_filelist = bkg_list
    for idx, f in enumerate(bkg_filelist):
        print('%d/%d' % (idx, len(bkg_filelist)))
        print(f"Running file {f} on gpu-{rank}")
        run_one_file(f, 0, model,rank)
    ...
    if(rank==0):
        ...
        for f in sorted(glob.glob(args.test_sig)):
            ...
            if mass:
                run_one_file(f, int(mass*1000), model,rank)  #-1)
            else:
                print("ERROR: unrecognized mass in filename {}".format(f))

def run_main(num_gpus=1):
    ...
    # List all files in the directory (sorted)
    all_files = sorted(glob.glob(args.test_bkg))
    # Determine the number of files each GPU should process
    num_files = len(all_files)
    chunk_size = (num_files + num_gpus - 1) // num_gpus  # This ensures all files are split evenly
    # Split files into contiguous chunks
    split_files = [all_files[i * chunk_size:(i + 1) * chunk_size] for i in range(num_gpus)]
    ...
    for i in range(num_gpus):
        if split_files[i]:  # Only start a process if there are files to process
            p = ctx.Process(target=run_filelist, args=(split_files[i], i))
            p.start()
            processes.append(p)
...
if __name__ == '__main__':
    ...
    run_main(int(os.environ["WORLDSIZE"]))
