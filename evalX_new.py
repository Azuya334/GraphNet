...
parser.add_argument('--nosig', action='store_true', default=False)
parser.add_argument('--nobkg', action='store_true', default=False)
...
# load data
input_dims = 6 if args.extended else 5 #4 # WAS: 5 (removed layer_id)
...
if not args.nosig:
    ...
    sig_filelist = sorted(glob.glob(args.test_sig))
    for idx, f in enumerate(sig_filelist):
        mass = None
        for m in masses.keys():
            if m in f:  
                mass = masses[m]
                print('%d/%d' % (idx, len(sig_filelist)))
                print("Running file", f)
