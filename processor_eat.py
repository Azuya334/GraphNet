...
output_dir = '/home/zwan/LDMX/LDMX-scripts/GraphNet/input/eat'
beam_8gev = True # set to false for v14 4gev
# Locations of the 2.3.0 ldmx-sw ROOT files to process+train on:

if beam_8gev:
    file_templates = {
        0.001:  '/home/zwan/LDMX/sigout/sig_eval/eat_mAMeV_0001_epsilon_0.01_minApE_2000_minPrimEatEcal_3500_Nevents_100000_run_3000.root',
        0.01:  '/home/zwan/LDMX/sigout/sig_eval/eat_mAMeV_0010_epsilon_0.01_minApE_2000_minPrimEatEcal_3500_Nevents_100000_run_3000.root',
        0.1:   '/home/zwan/LDMX/sigout/sig_eval/eat_mAMeV_0100_epsilon_0.01_minApE_2000_minPrimEatEcal_3500_Nevents_100000_run_3000.root',
        1.0:   '/home/zwan/LDMX/sigout/sig_eval/eat_mAMeV_1000_epsilon_0.01_minApE_2000_minPrimEatEcal_3500_Nevents_100000_run_3000.root',
        #0:     '/home/zwan/LDMX/sigout/pn/*.root'
    }
    ...
    with uproot.open(filename)['LDMX_Events'] as t:
        ...
        for branch in branchList:
            if not re.split('/', branch)[0] in t.keys():
                ...
                break
        if key_miss:
            print(f"MISSING KEYS IN: {filename}  SKIPPING... key {re.split('/', branch)[0]}, all keys {t.keys()}")
            return 0,0
        ...
        tskimmed_data = t.arrays(branchList)
        
        #print("Check tskimmed_data:")
        #print(tskimmed_data[blname('EcalScoringPlaneHits_v3_v13','pdgID_')])
        nTotalEvents = len(tskimmed_data[blname('EcalRecHits_eat', 'xpos_', sig)])
        if nTotalEvents == 0:
        ...
        el = (tskimmed_data[blname('EcalVeto_eat', 'nReadoutHits_', sig)] < MAX_NUM_ECAL_HITS) * (tskimmed_data[blname('EcalVeto_eat', 'summedTightIso_', sig)] < MAX_ISO_ENERGY) 

        preselected_data = {}
        for branch in branchList:
            preselected_data[branch] = tskimmed_data[branch][el]
        #print("Preselected data")
        nEvents = len(preselected_data[blname('EcalVeto_eat', 'summedTightIso_', sig)])
        ...
        pdgID_ = preselected_data[blname('TargetScoringPlaneHits_eat', 'pdgID_', sig)]
        z_     = preselected_data[blname('TargetScoringPlaneHits_eat', 'z_', sig)]
        px_    = preselected_data[blname('TargetScoringPlaneHits_eat', 'px_', sig)]
        py_    = preselected_data[blname('TargetScoringPlaneHits_eat', 'py_', sig)]
        pz_    = preselected_data[blname('TargetScoringPlaneHits_eat', 'pz_', sig)]
        tspRecoil = []
        ...
        x_data = preselected_data[blname('EcalScoringPlaneHits_eat','x_', sig)]
        xsp_data = preselected_data[blname('TargetScoringPlaneHits_eat','x_', sig)]
        E_data = preselected_data[blname('EcalRecHits_eat','energy_', sig)]
        HE_data = preselected_data[blname('HcalRecHits_eat', 'energy_', sig)]
        pe_data = preselected_data[blname('HcalRecHits_eat', 'pe_', sig)]
        for i in range(nEvents):
        ...
        for branch, var in vector_holders.items():
            ...
            if parent == 'EcalScoringPlaneHits_eat' or parent == 'EcalScoringPlaneHits_eat':
                tree.Branch(branchname, var, "{}[nSPHits]/F".format(branchname))
            elif parent == 'TargetScoringPlaneHits_eat' or parent == 'TargetScoringPlaneHits_eat':
                tree.Branch(branchname+'tsp_', var, "{}[nTSPHits]/F".format(branchname+'tsp_'))
            elif parent == 'EcalRecHits_eat' or parent == 'EcalRecHits_eat': 
