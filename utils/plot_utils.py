...
def plotROC(preds, truths, sample_weight=None, output=None, label='signal', sig_eff=1, bkg_eff=1, **kwargs):
    ...
    roc_auc = dict()
    acc = dict()
    #print(f"truth shapes {truths.shape} preds shape {preds.shape}")
    #print(truths)
...
def plot_acc(tacc, vacc, epoch=20,output=None):
    colors = ['#14AD0C', '#FF8F13'] # [green, orange]
    fig, ax = plt.subplots()
    x_vals = np.linspace(1, epoch, epoch)
    y_vals = [tacc, vacc]
    print(f"y_val[0] shape={len(y_vals[0])}, y_val[1] shape={len(y_vals[1])}")
...
def plot_loss(tloss, epoch=20,output=None):
    color = '#FF8F13'
    fig, ax = plt.subplots()
    x_vals = np.linspace(1, epoch, epoch)
